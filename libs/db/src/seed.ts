...
import { roles, permissions, rolePermissions } from "./schema/permissions";
...
async function seed() {
  ...
  // 3. Link Roles to Permissions (Idempotent Glue)
  ...
  const mapping: Record<string, string[]> = {
    ...
    MEMBER_VERIFIED: ["heritage:view", "profile:manage", "matrimony:search", "matrimony:contact", "official:view"],
    EXPERT: ["heritage:view", "profile:manage", "expert:publish_listing", "mentorship:respond"],
  ...
  for (const [roleName, permissionSlugs] of Object.entries(mapping)) {
    ...
    for (const slug of permissionSlugs) {
      const perm = await db.query.permissions.findFirst({ where: eq(permissions.slug, slug) });
      if (!perm) continue;
